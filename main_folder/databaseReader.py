...
def export_cameras_to_txt(database_path, output_file):
    # Abre la conexión a la base de datos
    db = COLMAPDatabase.connect(database_path)

    # Ejecuta una consulta SQL para seleccionar todas las cámaras
    cursor = db.execute("SELECT * FROM cameras")
    # Verifica si hay filas en el cursor
    # if cursor.fetchall():
    #     print("La consulta devolvió resultados.")
    #     #cursor.fetchall()
    # else:
    #     print("La consulta no devolvió resultados.")

    # Abre el archivo de salida para escribir en formato CSV
    with open(output_file, 'w', newline='') as csvfile:
        # Define el escritor CSV
        writer = csv.writer(csvfile)
        
        # Escribe el encabezado
        writer.writerow(['camera_id', 'model', 'width', 'height', 'params', 'prior_focal_length'])
        
        # Itera sobre los resultados y escribe cada fila en el archivo CSV
        for row in cursor:
            # Convierte el campo 'params' de blob a una lista de valores
            params = blob_to_array(row[4], np.float64).tolist()
            
            # Escribe la fila en el archivo CSV
            writer.writerow([row[0], row[1], row[2], row[3], params, row[5]])

    # Cierra la conexión a la base de datos
    db.close()

def export_images_to_txt(database_path, output_file):
    # Abre la conexión a la base de datos
    db = COLMAPDatabase.connect(database_path)
    ...
    # Abre el archivo de salida en modo de agregar para no sobrescribir lo que ya está escrito
    with open(output_file, 'a', newline='') as csvfile:
        # Define el escritor CSV
        writer = csv.writer(csvfile)
        
        # Escribir un comentario para marcar el inicio de las imágenes
        writer.writerow(["# Image list with two lines of data per image:",
                         "#   IMAGE_ID, QW, QX, QY, QZ, TX, TY, TZ, CAMERA_ID, NAME",
                         "#   POINTS2D[] as (X, Y, POINT3D_ID)"])
        
        # Itera sobre los resultados y escribe cada imagen en el archivo de salida
        for row in cursor:
            # Escribe la información de la imagen en el archivo de salida
            writer.writerow([row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9]])
            # Agrega una línea en blanco después de cada imagen
            writer.writerow([])
